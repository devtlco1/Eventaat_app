...
- **Filament UI layout standardization (Phase 8I)**:
  ...
  - **Root schema stacking**: **`App\Filament\Support\FilamentSchemaLayout::stackSections()`** forces Filament **`Schema`** root columns to **1** at all breakpoints so sibling **`Section`** components **stack vertically** (fixes “Basics beside Schedule”, “Details beside Capacity”, etc., which came from Filament’s default multi-column schema layout at **`lg+`**).
  - **Forms / infolists**: **`Section::compact()`**, section-internal **`Grid`** rows (often **3–4** columns on desktop for short fields), **`columnSpanFull()`** for long text/PDF/uploads — applied across Operations + Restaurant Setup resources (users/roles/permissions, restaurants/branches/seating/tables/staff, bookings/manual booking create, menus/categories/items helpers, events/offers/stories, reviews, support tickets, notification templates, booking notifications view, subscriptions/invoices/call logs) plus shared **`BranchAvailabilityRuleFormComponents`**, **`RestaurantMenuCategoryFormSchema`**, **`RestaurantMenuItemFormSchema`**, story **`RelationManagers`**, **`RestaurantStoryContentRepeater`**, relation-manager modal schemas, and Livewire **`RestaurantMenuStructured*Table`** edit actions.
  - **Worked examples**: Event nights (**Basics** row: title/slug/restaurant/branch; **Schedule & capacity**: status/booking mode/price, then starts/ends/capacity, then read-only seat metrics) and notification templates (**Template** row: event/active/channel/locale; title/body full width; **Notes** below).
...
- **Restaurant panel readiness audit (Phase 8J)**:
  - **`/restaurant`** Filament reviewed for **`restaurant_owner`**, **`branch_manager`**, and **`restaurant_host`** against **`RestaurantPanelScope`** + staff-assignment helpers (**unchanged** query rules).
  - **Navigation**: Operations resources use **distinct Heroicons** and **stable sort order** (bookings → event nights → menus → offers → stories → reviews → support tickets); Restaurant Setup uses icons differentiated by intent (storefront, branches, seating areas, tables, staff).
