...
- **Platform access management (Filament; complements Phase 8G)**:
  - **`Access Management`** group on **`/platform`**: **Users** (lists every account including mobile **`customer`** users; **`super_admin`** assigns Spatie roles and may set passwords; **`operations_admin`** may view/update profile fields **without** role or password controls), **Roles** (**`super_admin`** only; attach **`web`** permissions; core seeded roles **`super_admin`**, **`operations_admin`**, **`restaurant_owner`**, **`branch_manager`**, **`restaurant_host`**, **`customer`** cannot be deleted), **Permissions** (**`super_admin`** inspect-only list + view — **no** create/edit/delete). **No** mobile/public API changes or migrations for this UI-only layer.
