...
- **Restaurant menu foundation (Phase 16A)** (dashboard-only):
  ...
  - Structured mode: create flow shows a short **save-first** helper; **Edit menu** opens the **Menu builder** (per-category cards, each with a native Filament **items** table: header **Add item**, row edit/delete modals). PDF and external modes stay separate with mode-specific fields only. Categories relation manager is **disabled by default** and can be re-enabled via `RestaurantMenuStructuredUi::SHOW_CATEGORIES_RELATION_MANAGER_FALLBACK`
